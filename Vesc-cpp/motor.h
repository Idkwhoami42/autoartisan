...
class Controller {
    public:
        ...
        void goToPos(boost::asio::serial_port* serial, double pos, std::vector<Motor*> motors, double degrees_per_step=0.1, int acc_substeps=10, int velocity=40, int velocity_rampsteps=-1);

        void goToPosXY(boost::asio::serial_port* serial, double pos_x, double pos_y, Motor* horizontal_motor, std::vector<Motor*> vertical_motors, double degrees_per_step=0.1, int acc_substeps=10, int velocity=40, int velocity_rampsteps=-1);
};

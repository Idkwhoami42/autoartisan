...
// Constructor
Motor::Motor(std::string joint_name, const int id, boost::asio::serial_port* serial, const bool is_can) :
...
int32_t Motor::unpackI(const std::vector<uint8_t>& buffer, const int startIndex, const int endIndex) {
    if (endIndex - startIndex != 4 || startIndex < 0 || endIndex > static_cast<int>(buffer.size())) {
...
bool Motor::getValues(boost::asio::serial_port* serial) {
    ...
    if (this->homed) {
        this->pid_pos_now = ((unpackI(buffer, 7, 11) - 1e6*this->offset + 1e6*this->home_cooldown) / 1e6);
...
void Motor::setDerivativeGain(boost::asio::serial_port &serial, float gain) {
    ...
    std::vector<uint8_t> new_buffer2 = {3, new_buffer2_length >> 8, new_buffer2_length & 0xFF, COMM_FORWARD_CAN, static_cast<uint8_t>(this->id)};
...
void Controller::goToPos(boost::asio::serial_port* serial, const double pos, std::vector<Motor*> motors, const double degrees_per_step, int acc_substeps, const int velocity, int velocity_rampsteps) {
    ...
    if (motors.size() > 1) {
        ...
        for (const Motor* motor : motors) {
            if (std::abs(motor->pos - average_position) > 2) {
                ...
                std::cout << "Motor " << motor->joint_name << " is at " << motor->pos << std::endl;
                std::cout << "Average position is " << average_position << std::endl;
            }
        }
    ...
    if (mid_pos < pos) {
        while (mid_pos < pos) {
            ...
            const double current_velocity = (current_step <= velocity_rampsteps) ? ((static_cast<double>(velocity) / velocity_rampsteps) * current_step) : velocity;
    ...
    for (auto [pos, timestamp] : mid_positions) {
        ...
        if (const double elapsed = std::chrono::duration<double>(current_time - start_time).count(); new_timestamp > elapsed) {
            std::this_thread::sleep_for(std::chrono::duration<double>(new_timestamp - std::chrono::duration<double>((std::chrono::high_resolution_clock::now() - start_time)).count()));
        } else {
            missed++;
            continue;
        }
    ...
    std::cout << "Finished the loop with " << missed << " misses, with a total of " << mid_positions.size() << " steps" << std::endl;
...
void Controller::goToPosXY(boost::asio::serial_port* serial, const double pos_x, const double pos_y, Motor* horizontal_motor, std::vector<Motor*> vertical_motors, const double degrees_per_step, const int acc_substeps, const int velocity, int velocity_rampsteps) {
    std::cout << "Going to go to pos " << pos_x << ", " << pos_y << std::endl;

    this->goToPos(serial, pos_x, {horizontal_motor}, degrees_per_step, acc_substeps, velocity, velocity_rampsteps);
    this->goToPos(serial, pos_y, std::move(vertical_motors), degrees_per_step, acc_substeps, velocity, velocity_rampsteps);
